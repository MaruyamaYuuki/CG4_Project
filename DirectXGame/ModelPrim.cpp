...
/// 静的メンバ変数の実体
/// </summary>
const char* ModelPrim::kBaseDirectory = "Resources/";
const char* ModelPrim::kDefaultModelName = "cube";
ModelCommonPrim* ModelCommonPrim::sInstance_ = nullptr;

void ModelPrim::StaticInitialize() { ModelCommonPrim::GetInstance()->Initialize(); }

void ModelPrim::StaticFinalize() { ModelCommonPrim::GetInstance()->Terminate(); }

ModelPrim* ModelPrim::Create() {
	// メモリ確保
	ModelPrim* instance = new ModelPrim;
	instance->InitializeFromFile(kDefaultModelName, false);

	return instance;
}

ModelPrim* ModelPrim::CreateFromOBJ(const std::string& modelname, bool smoothing) {
	// メモリ確保
	ModelPrim* instance = new ModelPrim;
...
ModelPrim* ModelPrim::CreateSphere(uint32_t divisionVertial, uint32_t divisionHorizontal) {
	// メモリ確保
	ModelPrim* instance = new ModelPrim;
...
ModelPrim* ModelPrim::CreateSquare(int count) {
	ModelPrim* instance = new ModelPrim;
	std::vector<Mesh::VertexPosNormalUv> vertices;
	std::vector<uint32_t> indices;

	const float squareSize = 1.0f; // 1個の正方形の大きさ
	...
	for (int i = 0; i < count; ++i) {
		...
		int vi = i * kNumVertices;
		int ii = i * kNumIndices;
...
void ModelPrim::InitializeFromFile(const std::string& modelname, bool smoothing) {
	// モデル読み込み
...
void ModelPrim::InitializeFromVertices(const std::vector<Mesh::VertexPosNormalUv>& vertices, const std::vector<uint32_t>& indices) {
...
void ModelPrim::LoadModel(const std::string& modelname, bool smoothing) {
...
void ModelPrim::LoadMaterial(const std::string& directoryPath, const std::string& filename) {
	// ファイルストリーム
	std::ifstream file;
	// マテリアルファイルを開く
...
void ModelPrim::AddMaterial(std::unique_ptr<Material>& material) {
	// コンテナに登録
	materials_.emplace(material->name_, std::move(material));
}

void ModelPrim::LoadTextures() {
	int textureIndex = 0;
...
void ModelPrim::Draw(const WorldTransform& worldTransform, const Camera& camera, const ObjectColor* objectColor) {

	ModelCommonPrim* common = ModelCommonPrim::GetInstance();
...
void ModelPrim::Draw(const WorldTransform& worldTransform, const Camera& camera, uint32_t textureHadle, const ObjectColor* objectColor) {
...
void ModelPrim::SetAlpha(float alpha) {
...
ModelCommonPrim* ModelCommonPrim::GetInstance() {
	if (!ModelCommonPrim::sInstance_) {
		ModelCommonPrim::sInstance_ = new ModelCommonPrim();
	}
	return ModelCommonPrim::sInstance_;
}

void ModelCommonPrim::Terminate() {
	delete sInstance_;
	sInstance_ = nullptr;
}

void ModelCommonPrim::Initialize() {
	// パイプライン初期化
...
void ModelCommonPrim::LightCommand(const LightGroup* lightGroup) {
	// ライトコマンドを積む
	if (lightGroup) {
		lightGroup->Draw(commandList_, static_cast<UINT>(ModelPrim::RoomParameter::kLight));
	} else {
		defaultLightGroup_->Draw(commandList_, static_cast<UINT>(ModelPrim::RoomParameter::kLight));
	}
}

void ModelCommonPrim::TransformCommand(const WorldTransform& worldTransform, const Camera& camera) {
	// CBVをセット（ワールド行列）
	commandList_->SetGraphicsRootConstantBufferView(static_cast<UINT>(ModelPrim::RoomParameter::kWorldTransform), worldTransform.GetConstBuffer()->GetGPUVirtualAddress());

	// CBVをセット（カメラ）
	commandList_->SetGraphicsRootConstantBufferView(static_cast<UINT>(ModelPrim::RoomParameter::kCamera), camera.GetConstBuffer()->GetGPUVirtualAddress());
}

void ModelCommonPrim::PreDraw(ID3D12GraphicsCommandList* commandList) {
...
void ModelCommonPrim::PostDraw() {
	// コマンドリストを解除
	commandList_ = nullptr;
}

void ModelCommonPrim::InitializeGraphicsPipeline() {
