...
/// 静的メンバ変数の実体
/// </summary>
const char* ModelPrimitive::kBaseDirectory = "Resources/";
const char* ModelPrimitive::kDefaultModelName = "cube";
ModelCommonPrimitive* ModelCommonPrimitive::sInstance_ = nullptr;

void ModelPrimitive::StaticInitialize() { ModelCommonPrimitive::GetInstance()->Initialize(); }

void ModelPrimitive::StaticFinalize() { ModelCommonPrimitive::GetInstance()->Terminate(); }

ModelPrimitive* ModelPrimitive::Create() {
	// メモリ確保
	ModelPrimitive* instance = new ModelPrimitive;
	instance->InitializeFromFile(kDefaultModelName, false);

	return instance;
}

ModelPrimitive* ModelPrimitive::CreateFromOBJ(const std::string& modelname, bool smoothing) {
	// メモリ確保
	ModelPrimitive* instance = new ModelPrimitive;
...
ModelPrimitive* ModelPrimitive::CreateSphere(uint32_t divisionVertial, uint32_t divisionHorizontal) {
	// メモリ確保
	ModelPrimitive* instance = new ModelPrimitive;
...
ModelPrimitive* ModelPrimitive::CreateSquare() {
	// メモリ確保
	ModelPrimitive* instance = new ModelPrimitive;
	std::vector<Mesh::VertexPosNormalUv> vertices;
	std::vector<uint32_t> indices;
...
void ModelPrimitive::PreDraw(ID3D12GraphicsCommandList* commandList) { ModelCommonPrimitive::GetInstance()->PreDraw(commandList); }

void ModelPrimitive::PostDraw() { ModelCommonPrimitive::GetInstance()->PostDraw(); }

void ModelPrimitive::InitializeFromFile(const std::string& modelname, bool smoothing) {
	// モデル読み込み
...
void ModelPrimitive::InitializeFromVertices(const std::vector<Mesh::VertexPosNormalUv>& vertices, const std::vector<uint32_t>& indices) {
...
void ModelPrimitive::LoadModel(const std::string& modelname, bool smoothing) {
...
void ModelPrimitive::LoadMaterial(const std::string& directoryPath, const std::string& filename) {
	// ファイルストリーム
	std::ifstream file;
	// マテリアルファイルを開く
...
void ModelPrimitive::AddMaterial(std::unique_ptr<Material>& material) {
	// コンテナに登録
	materials_.emplace(material->name_, std::move(material));
}

void ModelPrimitive::LoadTextures() {
	int textureIndex = 0;
...
void ModelPrimitive::Draw(const WorldTransform& worldTransform, const Camera& camera, const ObjectColor* objectColor) {

	ModelCommonPrimitive* common = ModelCommonPrimitive::GetInstance();
...
void ModelPrimitive::Draw(const WorldTransform& worldTransform, const Camera& camera, uint32_t textureHadle, const ObjectColor* objectColor) {
...
void ModelPrimitive::SetAlpha(float alpha) {
...
ModelCommonPrimitive* ModelCommonPrimitive::GetInstance() {
	if (!ModelCommonPrimitive::sInstance_) {
		ModelCommonPrimitive::sInstance_ = new ModelCommonPrimitive();
	}
	return ModelCommonPrimitive::sInstance_;
}

void ModelCommonPrimitive::Terminate() {
	delete sInstance_;
	sInstance_ = nullptr;
}

void ModelCommonPrimitive::Initialize() {
	// パイプライン初期化
...
void ModelCommonPrimitive::LightCommand(const LightGroup* lightGroup) {
	// ライトコマンドを積む
	if (lightGroup) {
		lightGroup->Draw(commandList_, static_cast<UINT>(ModelPrimitive::RoomParameter::kLight));
	} else {
		defaultLightGroup_->Draw(commandList_, static_cast<UINT>(ModelPrimitive::RoomParameter::kLight));
	}
}

void ModelCommonPrimitive::TransformCommand(const WorldTransform& worldTransform, const Camera& camera) {
	// CBVをセット（ワールド行列）
	commandList_->SetGraphicsRootConstantBufferView(static_cast<UINT>(ModelPrimitive::RoomParameter::kWorldTransform), worldTransform.GetConstBuffer()->GetGPUVirtualAddress());

	// CBVをセット（カメラ）
	commandList_->SetGraphicsRootConstantBufferView(static_cast<UINT>(ModelPrimitive::RoomParameter::kCamera), camera.GetConstBuffer()->GetGPUVirtualAddress());
}

void ModelCommonPrimitive::PreDraw(ID3D12GraphicsCommandList* commandList) {
...
void ModelCommonPrimitive::PostDraw() {
	// コマンドリストを解除
	commandList_ = nullptr;
}

void ModelCommonPrimitive::InitializeGraphicsPipeline() {
